    ...
    else
    {
        ...
        var Make = function(team, arg)
        {
            ...
            for (i = 0; i < 11; ++i)
            {
                switch (team[i].Type)
                {
                    case 'bat':
                        ...
                        this.bat_average.push(parseFloat(team[i]['Average']));
                        this.bat_rating.push(parseInt(team[i]['Rating (900)']));
                        this.bat_strike_rate.push(parseFloat(team[i]['Strike Rate']));
                        this.average_bat_rating += parseInt(team[i]['Rating (900)']);
                        break;
                    case 'bowl':
                        this.bowl_index.push(i);
                        this.bat_name.push(team[i]['Name']);
                        this.bowl_name.push(team[i]['Name']);
                        this.economy.push(parseFloat(team[i]['Economy']));
                        this.bowl_average.push(parseFloat(team[i]['Avg']));
                        this.bowl_strike_rate.push(parseFloat(team[i]['SR']));
                        this.bat_average.push(parseFloat(team[i]['Average']));
                        this.bowler_rating.push(parseInt(team[i]['Rating (900)']));
                        this.bat_strike_rate.push(parseFloat(team[i]['Strike Rate']));
                        this.bat_rating.push(900 - parseInt(team[i]['Rating (900)']));
                        this.average_bowl_rating += parseInt(team[i]['Rating (900)']);
                        break;
                    case 'all':
                        this.bat_name.push(team[i]['Name']);
                        this.bowl_name.push(team[i]['Name']);
                        this.bat_rating.push(parseInt(team[i]['Bat']));
                        this.economy.push(parseFloat(team[i]['Economy']));
                        this.bowler_rating.push(parseInt(team[i]['Bowl']));
                        this.bowl_average.push(parseFloat(team[i]['Avg']));
            ...
            for (i = 0; i < 11; ++i)
            {
                if (i < 6)
                {
                    this.bowler_rating[i] += parseFloat(this.bowler_rating[i] ) / (this.bowl_name.length - 1) - parseFloat(this.average_bowl_rating) / (this.bowl_name.length * (this.bowl_name.length - 1)) + parseInt(this.coach_rating);
                }
                this.bat_rating[i] += parseFloat(this.bat_rating[i]) / (this.bat_name.length - 1) - parseFloat(this.average_bat_rating) / (this.bat_name.length * (this.bat_name.length - 1)) + parseInt(this.coach_rating);
        ...
        {
            desperation[0] = +(data.team[+toss_index].streak < 0) * (1 - team_object[+toss_index].bat_rating[strike[0]] / 1000) * data.team[+toss_index].streak * ((Total[0] + 1) / 5000) * mean_rating[+!toss_index] / team_object[+toss_index].bat_strike_rate[strike[0]];
            desperation[1] = +(data.team[+toss_index].streak < 0) * (1 - team_object[+toss_index].bat_rating[strike[1]] / 1000) * data.team[+toss_index].streak * ((Total[0] + 1) / 5000) * mean_rating[+!toss_index] / team_object[+toss_index].bat_strike_rate[strike[1]];
            for (i = 1; i < 6; i++)
            {
                if (team_object[+!toss_index].bowler_rating[i] > team_object[+!toss_index].bowler_rating[previous_bowler])
                {
                    previous_bowler = i;
                }
            }
            ...
            for (i = 0; i < 20 && (wickets[1] < 10 && (Total[1] <= Total[0] && loop)); ++i)
            {
                ...
                for (j = 1; j <= 6; ++j)
                {
                    delivery_score = team_object[+toss_index].bat_rating[strike[+strike_index]] - team_object[+!toss_index].bowler_rating[current_bowler];
                    bowler_performance_index = (team_object[+!toss_index].bowler_rating[current_bowler]) / ((rand(team_object[+!toss_index].bowl_average[strike[+strike_index]] * team_object[+!toss_index].bowler_rating[current_bowler] / bowl[0] + 1) + team_object[+!toss_index].bowl_average[current_bowler] * team_object[+!toss_index].bowler_rating[current_bowler] / 1000) * (rand(team_object[+!toss_index].bowl_strike_rate[current_bowler] * team_object[+!toss_index].bowler_rating[current_bowler] / bowl[1] + 1) + team_object[+!toss_index].bowl_strike_rate[current_bowler] * team_object[+!toss_index].bowler_rating[current_bowler] / 1000) * (rand(team_object[+!toss_index].economy[current_bowler] * team_object[+!toss_index].bowler_rating[current_bowler] / bowl[2] + 1) + team_object[+!toss_index].economy[current_bowler] * team_object[+!toss_index].bowler_rating[current_bowler] / 1000));
                    batsman_performance_index = (rand(team_object[+toss_index].bat_average[strike[+strike_index]] * team_object[+toss_index].bat_rating[strike[+strike_index]] / bat[0] + 1) + team_object[+toss_index].bat_average[strike[+strike_index]] * (1000 - team_object[+toss_index].bat_rating[strike[+strike_index]]) / 1000) * (rand(team_object[+toss_index].bat_strike_rate[strike[+strike_index]] * team_object[+toss_index].bat_rating[strike[+strike_index]] / bat[1] + 1) + team_object[+toss_index].bat_strike_rate[strike[+strike_index]] * (1000 - team_object[+toss_index].bat_rating[strike[+strike_index]]) / 1000) / team_object[+!toss_index].bowler_rating[current_bowler] + Math.pow(-1, (rand(2))) * ((frustration[strike_index] >= 3) ? frustration[strike_index] : 0);
                    if (!delivery_score)
                    {
                        delivery_score = 1;
                    }
                    ...
                    if (batsman_performance_index <= 0 && !free_hit)
                    {
                        ...
                        for(k = 0; k < 6; ++k)
                        {
                            temp += balls_faced[+strike_index][k] * team_object[+!toss_index].bowler_rating[k];
                        }
                        temp /= balls[strike[+strike_index]];
                        temp -= team_object[+toss_index].bat_rating[strike[+strike_index]];
                        temp += score[strike[+strike_index]] * (1 - Math.exp(-(Math.pow(fours[strike[+strike_index]], 1 / (1 - control[strike[+strike_index]])) + Math.pow(maximums[strike[+strike_index]], 1 / (1 - control[strike[+strike_index]]))) / (balls[strike[+strike_index]] + team_object[+toss_index].bat_rating[strike[+strike_index]])));
                        if(points < temp)
                        {
                            MoM.team = +toss_index;
                        ...
                        desperation[+strike_index] = +(data.team[+toss_index].streak < 0) * (1 - team_object[+toss_index].bat_rating[strike[+strike_index]] / 1000) * data.team[+toss_index].streak * ((Total[0] - Total[1] + 1) / 5000) * mean_rating[+!toss_index] / team_object[+toss_index].bat_strike_rate[strike[+strike_index]];
                        ...
                        fall_of_wicket = Total[+toss_index] + ' / ' + wickets[+toss_index] + ', ' + batsman_performance_index + '.' + temp;
                    }
                    else
                    {
                ...
                if(loop)
                {
                    data.match.commentary.push(', RRR: ' + parseFloat(((Total[0] + 1 - Total[1]) / (19 - i))).toFixed(2) + '  Equation: ' + data.team[+toss_index]._id + ' needs ' + (Total[0] + 1 - Total[1]) + ' runs from ' + (114 - 6 * i) + ' balls with ' + (10 - wickets[1]) + ' wickets remaining');
                }
                ...
                if (strike[+!strike_index] < 11)
                {
                    data.match.commentary.push(' ' + team_object[+toss_index].bat_name[strike[+!strike_index]] + ' : ' + score[strike[+!strike_index]] + ' (' + balls[strike[+!strike_index]] + '), Control: ' + control[strike[+!strike_index]] * 100 +  ' %, Partnership: ' + partnership_runs[current_partnership_index] + '(' + partnership_balls[current_partnership_index] + '), runrate: ' + (partnership_runs[current_partnership_index] * 6 / partnership_balls[current_partnership_index])).toFixed(2);
                }
                if (previous_batsman > -1)
                {
                    data.match.commentary.push(' Previous Wicket: ' + team_object[+toss_index].bat_name[previous_batsman] + ': ' + score[previous_batsman] + '(' + balls[previous_batsman] + ')' + 'Control: ' + control[previous_batsman] * 100);
                    if (previous_dismissal > -1)
                    {
                ...
                {
                    if (deliveries[j] <= 18 && team_object[+!toss_index].bowler_rating[j] > team_object[+!toss_index].bowler_rating[current_bowler] && j != previous_bowler)
                    {
                        current_bowler = j;
                    }
                }
            ...
            {
                if(data.team[+toss_index].squad[i] > 303)
                {
                    continue;
                }
                ...
                if((data.team[+toss_index].squad[i] > 0 && data.team[+toss_index].squad[i] < 114) || (data.team[+toss_index].squad[i] > 242 && data.team[+toss_index].squad[i] < 304))
                {
                    ...
                    data.team[+toss_index].stats[data.team[+toss_index].squad[i]].strike_rate = data.team[+toss_index].stats[data.team[+toss_index].squad[i]].runs_scored * 100 / data.team[+toss_index].stats[data.team[+toss_index].squad[i]].balls;
                    data.team[+toss_index].stats[data.team[+toss_index].squad[i]].average = data.team[+toss_index].stats[data.team[+toss_index].squad[i]].runs_scored / data.team[+toss_index].stats[data.team[+toss_index].squad[i]].outs;
                    ...
                    else if(data.team[+toss_index].stats[data.team[+toss_index].squad[i]].low > score[j])
                    {
            ...
            for(i in data.team[+!toss_index].squad)
            {
                if(data.team[+!toss_index].squad[i] > 113 && data.team[+!toss_index].squad[i] < 243)
                {
                    ...
                    data.team[+!toss_index].stats[data.team[+!toss_index].squad[i]].economy = data.team[+!toss_index].stats[data.team[+!toss_index].squad[i]].runs_given * 6 / data.team[+!toss_index].stats[data.team[+!toss_index].squad[i]].overs;
                    data.team[+!toss_index].stats[data.team[+!toss_index].squad[i]].avg = data.team[+!toss_index].stats[data.team[+!toss_index].squad[i]].runs_given / data.team[+!toss_index].stats[data.team[+!toss_index].squad[i]].wickets_taken;
                    data.team[+!toss_index].stats[data.team[+!toss_index].squad[i]].sr = data.team[+!toss_index].stats[data.team[+!toss_index].squad[i]].overs / data.team[+!toss_index].stats[data.team[+!toss_index].squad[i]].wickets_taken;
                }
            }
            ...
            for (i = 0; i < 11; ++i)
            {
                ...
                {
                    data.match.scorecard.push([team_object[+toss_index].bat_name[i], score[i], balls[i], (score[i] * 100 / balls[i]).toFixed(2), fours[i], maximums[i], dot_balls[i], control[i] * 100]);
                    if (!dismissed[i])
                    {
            ...
            data.match.scorecard.push('Total: ' + Total[+toss_index] + ' / ' + wickets[+toss_index] + ' (' + parseInt(Overs[+toss_index] / 6) + '.' + Overs[+toss_index] % 6 + ' overs)  Runrate: ' + (Total[+toss_index] * 6 / Overs[+toss_index]).toFixed(2) + ' Extras: ' + extras);
            data.match.scorecard.push(' Runs scored in boundaries: ' + temp + ' of ' + Total[+toss_index] + ' (' + (temp * 100 / Total[+toss_index]).toFixed(2) + ' %) ');
            ...
            for (i = 0; i < 6; i++)
            {
                ...
                temp /= deliveries[i];
                temp -= team_object[+!toss_index].bowler_rating[i];
                temp += (wickets[i] * 20) * (1 - Math.exp(-Math.pow((dot_deliveries[i] + 1) * 6 , wickets_taken[i]) / (team_object[+!toss_index].bowler_rating[i] + deliveries[i] + runs_conceded[i])));
                if(points < temp)
                {
                    MoM.team = +!toss_index;
                    points = Math.round(temp);
                ...
                data.match.scorecard.push([team_object[+!toss_index].bowl_name[i], parseInt(deliveries[i] / 6) + '.' + deliveries[i] % 6, maidens[i], wickets_taken[i], runs_conceded[i], (runs_conceded[i] * 6 / deliveries[i]).toFixed(2)]);
