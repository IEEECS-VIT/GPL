...
var strikeIndex; // used as the index of the array strike, to rotate strike between the two current batsmen at the crease. Reset after the innings is complete.
...
var hinge = (arg) => {return arg && overs || wickets}; // use wickets or overs to decide match winner if totals are equal
            ...
            for (i = 0; i < 20 && (wickets[+tossIndex] < 10 && (!loop || total[+!tossIndex] <= total[+tossIndex])); ++i) // loop through for 20 overs
            {
                ...
                for (j = 1; j < 7; ++j) // loop through for 6 deliveries.
                {
                    ...
                    else
                    {
                        ...
                        deliveryScore = (deliveryScore > -1) * deliveryScore; // set deliverScore to 0 if negative, else leave as is
